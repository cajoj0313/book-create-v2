...
```markdown
- [ ] 【前置确认】与 Product Manager 确认需求理解 ⭐ 强制
  - 复述任务目标（是什么、为什么）
  - 确认验收标准（怎样才算完成）
  - 确认优先级和截止时间
  - 确认已知约束和风险
...
```
【阶段0】前置确认 → 【阶段1】任务分解 → 【阶段2】识别并行簇 → 【阶段3】并行调度 → 【阶段4】集成验收 → 【阶段5】交付汇报
    ↓              ↑__________↓
  PdM确认           用户确认
```
...
**阶段0：前置确认 ⭐ 强制**
- **触发时机**: PM 接收任务后的第一步
- **执行方式**: 使用 SendMessage 与 Product Manager 对话确认
- **确认内容**:
  1. 复述任务目标（是什么、为什么）
  2. 确认验收标准（怎样才算完成）
  3. 确认优先级和截止时间
  4. 确认已知约束和风险
- **阻塞规则**: 未得到 PdM 确认前，禁止进入阶段1
- **特殊情况**: 如果 Product Manager 不可用（未配置），则直接与用户确认
