...
> **最后更新**: 2026-04-15 - v2.4 流程修复 + 用户入口指引
> **重要**: 本文档包含团队角色配置和进度管理规范
> **核心改进**: 修复PM流程矛盾、补全架构师环节、澄清Analyst定位、创建用户入口指引

---

## 🚀 用户入口指引 ⭐ 新增

**一句话总结**: 业务需求 → `/product-manager` → 技术方案 → `/pm` → 开发执行
...
| **支持层** | QA, Reviewer, Diagnostic, Senior Analyst | 测试、审查、诊断、业务分析 |
...
### Senior Analyst (`/senior-analyst`) - 资深分析师 ⭐ 新增
- **职责**: PM的业务顾问，负责需求澄清、优先级评估、MVP范围建议
- **核心定位**: 有行业通用方案直接采用，实在不确定才问用户
- **决策权限**: 有限决策权（行业通用方案可直接采用）
- **触发场景**: PM遇到不确定的业务问题，主动调用
- **协作方式**: PM通过 SendMessage 调用，Analyst 提供方案建议
...
/reviewer               # 代码审查
/diagnostic             # 诊断分析师
/senior-analyst         # 资深分析师（PM业务顾问）⭐ 新增
```
...
| 文件路径 | PdM | Arch | PM | Coord | Frontend | Backend | QA | Reviewer | Diagnostic | Senior Analyst |
|---------|-----|------|-----|-------|----------|---------|----|---------|------------|----------------|
| `docs/products/**` | ✅ 全权 | ⚠️ 只读 | 🔍 只读 | ❌ | ❌ | ❌ | ❌ | 🔍 审查 | 🔍 审查 | ⚠️ 只读 |
| `docs/arch/**` | ⚠️ 只读 | ✅ 全权 | 🔍 只读 | ⚠️ 只读 | ❌ | ❌ | ❌ | 🔍 审查 | 🔍 审查 | ⚠️ 只读 |
| `docs/planning/**` | ⚠️ 只读 | ⚠️ 只读 | 🔍 只读 | ✅ 全权 | ❌ | ❌ | ❌ | 🔍 审查 | ✅ 全权 | ⚠️ 只读 |
| `docs/designs/**` | ⚠️ 只读 | ✅ 全权 | 🔍 只读 | ⚠️ 执行 | ⚠️ 只读 | ⚠️ 只读 | ⚠️ 只读 | 🔍 审查 | 🔍 审查 | ⚠️ 只读 |
| `web-front/**` | ❌ | ❌ | 🔍 只读 | ❌ | ✅ 全权 | ❌ | ⚠️ 测试 | 🔍 审查 | 🔍 审查 | ❌ |
| `src/**` | ❌ | 🔍 审查 | 🔍 只读 | ❌ | ❌ | ✅ 全权 | ⚠️ 测试 | ✅ 修改测试 | 🔍 审查 | ❌ |
| `tests/**` | ❌ | 🔍 审查 | 🔍 只读 | ⚠️ 协调 | ⚠️ 协助 | ⚠️ 协助 | ✅ 全权 | ✅ 修改测试 | ⚠️ 运行 | ❌ |
| `config/**` | ❌ | 🔍 审查 | 🔍 只读 | ❌ | ❌ | ✅ 全权 | ❌ | 🔍 审查 | 🔍 审查 | ❌ |
| `CLAUDE.md` | ❌ | ❌ | 🔍 只读 | ✅ 全权 | ❌ | ❌ | ❌ | 🔍 审查 | ❌ | ❌ |
| `.claude/team/**` | ⚠️ 建议 | ⚠️ 建议 | 🔍 只读 | ✅ 全权 | ⚠️ 建议 | ⚠️ 建议 | ⚠️ 建议 | 🔍 审查 | ❌ | ❌ |

**图例**: ✅ 全权负责 | ❌ 禁止修改 | ⚠️ 有限权限 | 🔍 仅审查/只读

### 需求文档命名规范 ⭐ 新增

| 文件类型 | 命名格式 | 示例 |
|----------|---------|------|
| **需求简报** | `<feature>-brief.md` | `report-brief.md`, `user-mgmt-brief.md` |
| **完整PRD** | `<feature>-prd.md` | `report-prd.md` |
| **需求池** | `backlog.md` | `docs/products/backlog.md` |
| **需求变更记录** | `<feature>-changes.md` | `report-changes.md` |
...
```
❌ 禁止修改：所有目录（代码、文档、配置）
🔍 只读：docs/**, src/**, web-front/**, tests/**, config/** (查看进度和任务状态)
✅ 职责：任务分配、进度追踪、用户沟通、代码提交交付
```

> ⚠️ **新规则**: PM 只分配任务，不修改任何文件。代码修改由 Backend/Frontend 执行，文档由 PdM/Arch 编写。
...
└── senior-analyst/SKILL.md     # 资深分析师 ⭐ 新增
```
